__author__ = 'alefur'
from pfsGUIActor.cam import CamDevice, ExposureState
from pfsGUIActor.control import CommandsGB
from pfsGUIActor.widgets import ValueGB, CustomedCmd, DoubleSpinBoxGB, SpinBoxGB
...
class BiasCmd(CustomedCmd):
    ...
    def buildCmd(self):
        cmdStr = 'spsait bias duplicate=%d cam=%s name="Functional Test" comments="took from pfsGUIActor"' % \
...
class DarksCmd(CustomedCmd):
    ...
    def buildCmd(self):
        cmdStr = 'spsait dark exptime=%.2f duplicate=%d cam=%s name="Functional Test" comments="took from pfsGUIActor"' % \
